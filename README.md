...
To set up dbcreate, you need three things:
...
the exact Python version required by dbcreate,
...
To execute dbcreate, run:

```
poetry run poe cli
```

## Contributing to dbcreate
...
```
poetry run poe tests -vv tests/test_db.py::test_hello
```
...
### Checking dbcreate’s dependencies for vulnerabilities

To check dbcreate’s dependencies for known vulnerabilities, run:

```
poetry run poe check
```

### Checking dbcreate’s dependencies for compatible updates

To check dbcreate’s dependencies for compatible updates, run:
...
Copyright (c) 2023 Abdulazeez Sherif
